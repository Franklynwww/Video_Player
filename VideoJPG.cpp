...
void VideoJPG::doJpgGet()
{
    ...
    for (int i = 0; i<in_fmt->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {   // 说明是视频
            ...
            if (!de_ctx) {
                printf("video avcodec_alloc_context3 fail");
                releaseSources();
                return;
            }
            ...
            if (avcodec_parameters_to_context(de_ctx, stream->codecpar) < 0) {
                printf("video avcodec_parameters_to_context");
                releaseSources();
                return;
            }
            ...
            if (avcodec_open2(de_ctx, codec, NULL) < 0) {
                printf("video avcodec_open2() fail");
                releaseSources();
                return;
            }
            break;
    ...
    if (av_frame_make_writable(en_frame) < 0) {
        printf("av_frame_make_writeable fail");
        releaseSources();
        return;
    }
...
//        printf("video pts %d(%s)",in_pkt->pts,av_ts2timestr(in_pkt->pts,&in_stream->time_base));
        while (true) {
             ...
             */
            printf("sucess pts %d",de_frame->pts);
            // 成功解码出来了
#if Get_More
            // 取多帧视频并写入到文件
            static int i=0;
...
{
    string curFile(__FILE__);
//    unsigned long pos = curFile.find("D:\\2-video_audio_advanced");
//    if (pos == string::npos) {
//        printf("not find file");
//        return;
//    }
...
//    string srcDic = curFile.substr(0,pos) + "filesources/";
    string srcPath = string("D:\\images\\0.jpg");
    string dstPath = "D:\\1-dojpgToVideo.mp4";
    int video_index = -1;
    ...
    for (int i=0; i<in_fmt->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            AVCodec *codec = avcodec_find_decoder(stream->codecpar->codec_id);
            if (!codec) {
                printf("not find jpg codec");
                releaseSources();
                return;
            }
            de_ctx = avcodec_alloc_context3(codec);
            if (!de_ctx) {
                printf("jpg codec_ctx fail");
                releaseSources();
                return;
            }
            ...
            if (avcodec_parameters_to_context(de_ctx, stream->codecpar) < 0) {
                printf("set jpg de_ctx parameters fail");
                releaseSources();
                return;
            }
...
//        printf("video pts %d(%s)",ou_pkt->pts,av_ts2timestr(ou_pkt->pts, &stream->time_base));
